 ...
 */
Steps.prototype.create = function (data) {
    let that = this;
    let time = new Date();
    let stepModel = Object.assign({}, Models.BL_MODEL_STEP, {
        create: time,
        update: time,
        workflow: new ObjectID(data.workflow_id),
        user: data.user
    }, data.step_data);

    return new Promise(function (resolve, reject) {
        that.stepCollection.insertOne(stepModel).then(function (__unused__stepResult) {
            that.workflowCollection.findOne({ _id: new ObjectID(data.workflow_id) }).then(function (foundWorkflow) {
                ...
                let stepGraph = Object.assign({}, {
                    id: stepModel._id,
                    parent: data.step_data.parent,
                    children: []
                });
                foundWorkflow.graph = that._graphInsert(foundWorkflow.graph, stepGraph);
                foundWorkflow.update = time;
                that.workflowCollection.findOneAndReplace({ _id: new ObjectID(data.workflow_id) }, foundWorkflow, { upsert: true }).then(function (__unused__updatedWorkflow) {
                    resolve(stepModel);
...
Steps.prototype.updateByID = function (step_id, step_data) {
    ...
    return new Promise(function (resolve, reject) {
        ...
        delete updated_step._id;
        delete updated_step.create;
        delete updated_step.workflow;
        delete updated_step.user;
